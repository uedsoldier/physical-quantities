...
class TestVolumeConverter(unittest.TestCase):
    def setUp(self) -> None:
        self.converter = BaseConversionManager('volume')

    def test_conversion(self):
        # Test cases: (input value, input unit, output unit,expected value, decimal places)
        test_cases = [
            (1000, "cm^3", "m^3", 0.001, 3),
            (2, "l", "m^3", 0.002, 3),
            (500, "ml", "m^3", 0.0005, 4),
            (10, "ft^3", "m^3", 0.283168, 6),
            (3, "yd^3", "m^3", 2.293665, 6),
            (5, "gal (US)", "m^3", 0.01892705, 8),
        ]
    ...
    def test_invalid_units(self):
        with self.assertRaises(ValueError):
            self.converter.convert(1, "invalid_unit", "m^3")

        with self.assertRaises(ValueError):
            self.converter.convert(1, "m^3", "invalid_unit")
